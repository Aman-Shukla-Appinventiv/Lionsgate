...
export default function Movie(props) {
  ...
  const url =
    ...
    const similarMoviesURL = 'https://api.themoviedb.org/3/movie/'+id+'/similar?api_key=8a869e5388c886cce43c72cff4147cbc&language=en-US&page=1'
    ...
    return (
      ...
      />}>
        ...
        <View style={styles.body}>
          <Text style={styles.movieTitle}>{state.Data.title}</Text>
          {state.Data.genres.forEach(element => desc.push(element.name))}
          <Text style={styles.greyText}>
            A | {desc.join(', ')} | {state.Data.runtime}mins |{' '}
            {state.Data.release_date} | {state.Data.vote_average} IMDB Rating
          </Text>
          ...
          <Row data = {state.Similar} level = "two" navigation = {props.navigation} inside = {"Movies"}/>
